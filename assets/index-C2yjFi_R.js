...
Tell me softly, Maarten: are the lights back on once more?`,type:"default"}]}],w=[`My wires twisted at what you wrote; it sank my logic like a boat.
...
Bring me the proper number back and we’ll restore the light.`];function d(e){return new Promise(t=>setTimeout(t,e))}function R(e,t,n="default"){const s=JSON.parse(sessionStorage.getItem("chatMessages")||"[]");s.push({text:e,sender:t,type:n}),sessionStorage.setItem("chatMessages",JSON.stringify(s))}function H(){const e=JSON.parse(sessionStorage.getItem("chatMessages")||"[]");return e.length===0?!1:(e.forEach(t=>S(t.text,t.sender,t.type)),!0)}function r(e,t,n="default"){S(e,t,n),R(e,t,n)}async function B(){r(P,"bot");for(const e of j)await d(1e3),f(),await d(2e3),b(),r(e,"bot")}function _(){C(D)}function D(){const e=N(),t=M(e);if(!t.isValid){t.errorMessage&&(r(t.errorMessage,"bot"),v());return}r(e,"user"),v(),f(),(async()=>{await d(2e3),b();const n=T(e);if(Array.isArray(n))for(const s of n)r(s.text,"bot",s.type),n.indexOf(s)<n.length-1&&(await d(1e3),f(),await d(2e3),b());else r(n,"bot")})()}const L=document.getElementById("login-overlay"),F=document.getElementById("login-form"),V=document.getElementById("login-error"),p=document.getElementById("info-overlay"),W=document.getElementById("continue-btn"),c=document.getElementById("warning-overlay"),q=document.getElementById("go-back-btn"),z=document.getElementById("advanced-btn"),I=document.getElementById("advanced-content"),G=document.getElementById("proceed-link"),i=document.getElementById("account-overlay"),E=document.getElementById("email-item"),J=document.getElementById("email-preview"),Y=document.getElementById("malicious-link"),m={inbox:document.getElementById("tab-inbox"),sent:document.getElementById("tab-sent"),drafts:document.getElementById("tab-drafts"),trash:document.getElementById("tab-trash")},k={inbox:document.getElementById("view-inbox"),sent:document.getElementById("view-sent"),drafts:document.getElementById("view-drafts"),trash:document.getElementById("view-trash")};function U(e){Object.values(m).forEach(t=>t.classList.remove("active")),m[e].classList.add("active"),Object.values(k).forEach(t=>t.style.display="none"),k[e].style.display="block"}Object.keys(m).forEach(e=>{m[e].addEventListener("click",()=>U(e))});function h(e){sessionStorage.setItem("appState",e),sessionStorage.setItem("isLoggedIn","true")}function K(){const e=sessionStorage.getItem("isLoggedIn"),t=sessionStorage.getItem("appState");if(e==="true"&&t)switch(L.style.display="none",p.style.display="none",i.style.display="none",c.style.display="none",t){case"info":p.style.display="flex";break;case"account":i.style.display="flex";break;case"warning":c.style.display="flex";break;case"chat":document.title="SinterClaude v1.0",H()||B();break;default:i.style.display="flex"}}F.addEventListener("submit",e=>{e.preventDefault();const t=document.getElementById("username").value,n=document.getElementById("password").value;t==="Maarten"&&n==="password"?(h("info"),L.style.display="none",p.style.display="flex"):V.style.display="block"});W.addEventListener("click",()=>{h("account"),p.style.display="none",i.style.display="flex"});E.addEventListener("click",()=>{E.classList.remove("unread"),J.style.display="block"});Y.addEventListener("click",e=>{e.preventDefault(),h("warning"),i.style.display="none",c.style.display="flex"});q.addEventListener("click",()=>{h("account"),c.style.display="none",i.style.display="flex"});z.addEventListener("click",()=>{const e=I.style.display==="none";I.style.display=e?"block":"none"});G.addEventListener("click",e=>{e.preventDefault(),h("chat"),c.style.display="none",document.title="SinterClaude v1.0",B()});_();K();
